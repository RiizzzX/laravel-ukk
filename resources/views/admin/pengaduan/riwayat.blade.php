        ...
        <table class="w-full">
          <thead>
            <tr class="bg-emerald-50 border-b border-emerald-100">
              ...
              <th class="text-left px-6 py-4 text-sm font-semibold text-emerald-900">Aksi</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            @forelse($pengaduan as $p)
              <tr class="hover:bg-emerald-50/30 transition">
                ...
                <td class="px-6 py-4">
                  <form action="{{ route('admin.pengaduan.destroy', $p->id_pengaduan) }}" method="POST" class="inline" onsubmit="return confirm('Yakin hapus riwayat pengaduan ini?')">
                    @csrf
                    @method('DELETE')
