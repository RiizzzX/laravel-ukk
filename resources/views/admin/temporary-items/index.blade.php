        ...
        <div class="relative p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-extrabold text-yellow-900 mt-1">{{ $pendingCount }}</p>
                </div>
            ...
            <div class="mt-4">
                <div class="h-2 w-full bg-yellow-200 rounded-full overflow-hidden">
                    @php
                        $totalTempItems = $pendingCount + $approvedCount + $rejectedCount;
                        $pendingPercentage = $totalTempItems > 0 ? ($pendingCount / $totalTempItems) * 100 : 0;
                    @endphp
        ...
        <div class="relative p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-extrabold text-emerald-900 mt-1">{{ $approvedCount }}</p>
                </div>
        ...
        <div class="relative p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-extrabold text-rose-900 mt-1">{{ $rejectedCount }}</p>
                </div>
            ...
            <div class="mt-4">
                <div class="h-2 w-full bg-rose-200 rounded-full overflow-hidden">
                    @php
                        $rejectedPercentage = $totalTempItems > 0 ? ($rejectedCount / $totalTempItems) * 100 : 0;
                    @endphp
                    <div class="h-full bg-rose-500 rounded-full" style="width: {{ $rejectedPercentage }}%"></div>
                </div>
                <p class="text-xs text-rose-700 mt-1">{{ number_format($rejectedPercentage, 1) }}% dari total</p>
            </div>
        </div>
    </div>
</div>
          ...
          @forelse($temporaryItems as $item)
            <tr class="hover:bg-purple-50/30 transition-colors">
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="text-sm font-bold text-purple-600">#{{ $item->id_temporary_item }}</span>
              </td>
              <td class="px-6 py-4">
                <div class="text-sm font-semibold text-gray-900">{{ $item->nama_item }}</div>
                @if($item->deskripsi)
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm text-gray-700">{{ $item->lokasi->nama_lokasi ?? '-' }}</span>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm text-gray-700">{{ $item->creator->username ?? '-' }}</span>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                @if($item->status === 'pending')
                  <div class="flex items-center gap-2">
                    <button onclick="approveItem({{ $item->id_temporary_item }})" 
                    ...
                    <button onclick="openRejectModal({{ $item->id_temporary_item }}, '{{ $item->nama_item }}')" 
